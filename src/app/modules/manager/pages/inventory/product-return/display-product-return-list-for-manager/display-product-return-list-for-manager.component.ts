...
export class DisplayProductReturnListForManagerComponent implements OnInit {
  ...
  loadList(): any {
    ...
    this._httpService
      .get(APIEndpoint.GET_PRODUCT_RETURN_LIST_FOR_MANAGER, this.payload)
      .pipe(
  ...
  handleAdd(): any {
    this._router.navigate(['../create-product-return'], {
      relativeTo: this._activatedRoute,
    });
  }

  handleView(value: any): any {
    this._router.navigate([`../view-product-return/${value}`], {
      relativeTo: this._activatedRoute,
      state: { edit: false },
    });
  }
